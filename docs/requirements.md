...
MVP の初期値は `provider=gemini`、`model=gemini-3.1-flash-lite`、`max output tokens=300`、`memory depth=2` とする。Pebble画面で読みやすいよう、system instruction では「日本語で1から3文、必要なら箇条書き2点まで」と指定する。

### 4.5 ChatGPT OAuth / 非公式方式の扱い
