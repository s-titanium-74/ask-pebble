...
Pebble Time 2 で音声入力を行い、OpenRouter API 経由の AI 回答を短文で時計に表示する PoC を開発する。MVP では OpenRouter の model-specific routing を使い、まずは Pebble と Android phone だけで「話す、送る、読む」までの体験を成立させる。backend を持たない BYOK (Bring Your Own Key) 方式で実装可能性を確認する。
...
- MVP provider route: model-specific。Speed は Groq only / fallback off、Balance と Quality は OpenRouter default routing
...
実装前ゲートとして、Android の Pebble mobile app 内で動く PebbleKit JS から OpenRouter API へ `Authorization` header 付き HTTPS request を送信できることを最初に確認する。あわせて Speed model で OpenRouter の provider routing による Groq only / fallback off の指定が成立するか確認する。この確認が通らない場合、backend なし BYOK 構成は MVP の第一候補から外し、Cloudflare Worker または local backend 経由の構成を MVP 代替案として再評価する。
...
MVP の dropdown 候補は、`Speed (Groq GPT-OSS 20B)`、`Balance (GPT-5 Mini)`、`Quality (Claude Haiku 4.5)` とする。対応する model id は `openai/gpt-oss-20b`、`openai/gpt-5-mini`、`anthropic/claude-haiku-4.5`。手入力欄に値がある場合は手入力値を優先し、空の場合は dropdown の選択値を使う。手入力値が無効、権限なし、または provider API で拒否された場合は、時計には `model_failed`、`provider_failed`、`auth_failed` 相当の短いエラーを表示し、設定画面で model id の確認を促す。

MVP の provider 情報は設定画面に read-only で表示する。表示内容は `Provider: OpenRouter`、`Routing: Model-specific`、`Speed route: Groq only` とし、MVP では変更不可とする。
...
- AppMessage の key は、`type`、`requestId`、`status`、`answer`、`errorCode`、`message`、`hasApiKey` に加え、Health context 用の `stepsToday`、`activeMinutesToday`、`sleepTodayMinutes`、`restfulSleepTodayMinutes`、`healthAvailable` を使う。
...
### 3.6 端末コンテキストと擬似 tool

LLM 呼び出しは 1 user turn あたり最大 2 回に制限する。

- 初回呼び出しだけ、利用可能な擬似 tool を system instruction に含める。
- tool request は `{"tools":["location","health"],"reason":"brief"}` のような JSON object とする。`tools` は複数指定できる。
- 初回応答に JSON が含まれ、tool request として解釈できない場合は tool 失敗扱いとし、context なしで 2 回目の LLM 呼び出しを行う。
- 2 回目呼び出しには tool instruction を含めない。2 回目応答に JSON が含まれても追加 tool 実行はしない。
- Time context は default ON。Location context と Health context は default OFF。
- Health context は PebbleKit JS から直接読まず、watchapp C 側の `HealthService` で取得して AppMessage で返す。
- Health context の初期対象は、今日の歩数、今日の active minutes、今日の sleep minutes、今日の restful sleep minutes に絞る。

Decision record:

- A standalone router-first flow was tested but not adopted.
- Verification showed `openai/gpt-5-mini` could classify `今日の歩数を教えて` as `tools=[]` in standalone router mode, while the combined first-call prompt correctly requested `health`.
- The app therefore uses a combined first call: answer normally if no context is needed, or return tool-request JSON if context is needed.
- GPT-5-family models require reasoning suppression (`reasoning.effort=minimal`, `reasoning.exclude=true`, `reasoning_effort=minimal`) to avoid `finish=length` with `content=null`.
...
MVP は OpenRouter BYOK 方式を採用する。ユーザーは Pebble app の設定画面で自分の OpenRouter API key を入力し、PebbleKit JS がその key を使って OpenRouter API を呼び出す。OpenRouter の provider routing は選択モデルごとに扱い、Speed model では Groq only / fallback off に固定する。
...
MVP の provider は OpenRouter API とし、選択モデルに応じて provider routing を変える。理由は、OpenRouter API key だけで model id と provider routing を扱え、Groq の高速応答、GPT のバランス、Claude の品質を同じ UI で比較できるためである。

初期実装では provider adapter の汎用化を必須にしない。まず OpenRouter 固定、routing は model-specific で、Dictation から回答表示までの体験を完成させる。
...
| --- | --- | --- | --- |
| OpenRouter -> Groq only | Speed | `openai/gpt-oss-20b` | 最低コスト・高速寄り。時計向け短文回答のデフォルト候補。 |
| OpenRouter default | Balance | `openai/gpt-5-mini` | コストを抑えつつ、回答品質と安定性を上げたい場合の候補。 |
| OpenRouter default | Quality | `anthropic/claude-haiku-4.5` | 3候補内では高コストだが、回答品質を優先する候補。 |
| Direct Groq or OpenRouter拡張 | v1 web search利用 | `groq/compound-mini` 等 | Web search を使う候補。通常Q&Aより応答時間とコストを別途検証する。 |

MVP の初期値は `provider=openrouter`、`route=model_specific`、`speed_route=groq_only`、`speed_fallbacks=off`、`model=openai/gpt-oss-20b`、`language=Auto`、`max output tokens=300`、`memory depth=2`、`timeout seconds=12` とする。Pebble画面で読みやすいよう、system instruction では小さなスマートウォッチ画面向けに簡潔で実用的な回答を返すよう指定する。
...
| OpenRouter で model-specific routing を使う | 要検証 / MVP本線 | Speed は Groq only / fallback off、他は OpenRouter default routing とする。 |
| Direct Groq API を使う | v2以降 / 代替案 | MVPでは本線にしない。OpenRouter経由との速度・費用・安定性比較候補。 |
| Gemini / OpenAI / Anthropic API を使う | v2以降 | MVPでは direct API 対応しない。OpenRouter経由で体験が成立した後に追加検討する。 |
...
- 日本語短文回答に最適化する。
- OpenRouter API key を使いつつ、model-specific routing で高速応答と品質差を検証する。
...
1. BYOK 設定保存と provider 呼び出し
   ...
   - Speed model で OpenRouter の provider routing による Groq only / fallback off を指定できるか。
